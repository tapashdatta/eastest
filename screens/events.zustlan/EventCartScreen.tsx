// screens/events/EventCartScreen.tsx - Modern EventCartScreen with React 19
...
import { useEventCart } from '@/stores/eventStore';
...
interface CartItemProps {
  item: any;
  onRemove: (itemId: string, eventTitle: string) => void;
  onUpdateQuantity: (itemId: string, quantity: number) => void;
}

const CartItem: React.FC<CartItemProps> = React.memo(({ item, onRemove, onUpdateQuantity }) => {
  const handleRemove = useCallback(() => {
    onRemove(item.id, item.event_title);
  }, [item.id, item.event_title, onRemove]);
...
const EmptyCartView: React.FC<{ onContinueShopping: () => void }> = React.memo(({ onContinueShopping }) => (
...
const OrderSummary: React.FC<{ cartTotals: any }> = React.memo(({ cartTotals }) => (
...
const EventCartScreen: React.FC = () => {
  ...
  const { cart, removeFromCart, updateQuantity, cartTotals, isCartEmpty } = useEventCart();
  ...
  // Item handlers
  const handleRemoveItem = useCallback((itemId: string, eventTitle: string) => {
    Alert.alert(
      'Remove Event',
  ...
  const cartItems = useMemo(() => 
    ...
    [cart, handleRemoveItem, handleUpdateQuantity]
  );
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {isCartEmpty ? (
          <EmptyCartView onContinueShopping={handleContinueShopping} />
        ) : (
          <>
            <View style={styles.cartItems}>
              {cartItems}
            </View>
            <OrderSummary cartTotals={cartTotals} />
          </>
        )}
      </ScrollView>
